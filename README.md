# Project 6 - NCTU_MAP
#### 在這張交大地圖可行走之道路上選取任意兩點，找出兩點間的最短路徑，並且以平滑曲線顯示。
## 檔案有:
+ *編輯過照片 \*4*
...
+ 假設source為A，D(a, b)表示a,b目前最短距離，共有n點。
...
+ 之後的每次迴圈都找出一個current node，做同樣的判斷及更新，在所有D(A, i)都=9999時，就代表我找到所有n-1個節點到A的最短距離了，可跳出迴圈。 (理論上，應該跑n-1次迴圈即可)
---
