...
## 主要目標: (由低到高)
1. 在此圖上可行走之主要道路上任意點兩點，可找出一條路徑通過，且此路徑在可行走之主要道路上。 *(完成)*
2. 路徑上的節點不重複，且此路徑必須為最短路徑。 *(完成)*
3. 找出高次多項式逼近路線，使其顯示出來是一條平滑曲線，而非點到點的折線圖。 *(完成)*
***
4. 增加一些功能，可能有: 重複執行 *(完成)*、一開始顯示可行走的地方、顯示選取的點是否有被接受...。
...
8. (待想)
## 實踐方法:
1. 將可行走之主要道路區分成眾多節點和邊，形成一張**simple graph**，並記錄「各編號節點的座標」、「各兩點間距離」。
